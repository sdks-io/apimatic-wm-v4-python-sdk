...
def _head_response(streamed: StreamedResponse | AsyncStreamedResponse, request: HttpRequest) -> HttpResponse:
    ...
    return HttpResponse(status_code=streamed.status_code, headers=dict(streamed.headers), content=b"", request=request)
        ...
        *,
        ...
        try:
            ...
            if 200 <= streamed.status_code < 300:
                ...
                return Success(
                    payload=decoder.decode(streamed, request.url), response=_head_response(streamed, request)
                )
